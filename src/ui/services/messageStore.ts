...
export class InMemoryMessageStore implements MessageStore {
  ...
  addMessage(message: MessageViewModel): boolean {
    ...
    if (isNew) {
      ...
      if (shouldUpdate) {
        // Merge state: preserve created_at and sender_id from original (first message is authoritative)
        // Update other fields from incoming message (state, display_state, etc.)
        conversationMessages.set(messageId, {
          ...
          // Preserve sender_id from original (deduplication by ID only, not by sender)
